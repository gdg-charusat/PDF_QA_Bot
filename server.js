...
// ENHANCEMENT: Stream endpoint for real-time token streaming (Issue #118)
app.post("/ask-stream", askLimiter, async (req, res) => {
  const { question, session_ids } = req.body;
  ...
  try {
    ...
    // Pipe the FastAPI stream directly to client
    ...
    console.error("[/ask-stream]", error.response?.data || error.message);
    res.setHeader("Content-Type", "application/json");
    return res.status(500).json({ error: "Error streaming answer." });
  }
});
